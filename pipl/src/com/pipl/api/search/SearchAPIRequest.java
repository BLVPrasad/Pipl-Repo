...
public class SearchAPIRequest {
	 ...
	 *            missing.
	 */
	public String validateQueryParams(boolean strict) //throws IllegalArgumentException 
	{
		String msg = "";
		...
		if (strict
				&& (!"and".equals(queryParamsMode) && !"or" .equals(queryParamsMode))) {
			//throw new IllegalArgumentException("queryParamsMode should be one of and/or");
			msg = "queryParamsMode should be one of and/or";
			
		}
		if (!person.isSearchable()) {
			//throw new IllegalArgumentException("No valid name/username/phone/email in request");
			msg = "No valid name/username/phone/email in request";
			
		}
		if (strict && !person.unsearchableFields().isEmpty()) {
			//throw new IllegalArgumentException("Some fields are unsearchable: "+ person.unsearchableFields());
			msg = "Some fields are unsearchable: "+ person.unsearchableFields();
		}
		return "Sorry..."+msg;
	}
	 ...
	 * @throws URISyntaxException
	 */
	public ArrayList send(boolean strictValidation)
			throws SearchAPIError, IOException, URISyntaxException {
		String validation = validateQueryParams(strictValidation);
		...
		if(validation.equals("Sorry..."))
		{
		try {
			
			//HttpHost proxy = new HttpHost("hexproxy.hexaware.local", 3128,"http");
			...
			if (httpResponse.getStatusLine().getStatusCode() == HttpStatus.SC_OK) {
				//return responseBody;
				//System.out.println("response : "+(SearchAPIResponse) Utils.fromJson(responseBody,SearchAPIResponse.class));
				list.add((SearchAPIResponse) Utils.fromJson(responseBody,SearchAPIResponse.class));
				return list;
				//return (SearchAPIResponse) Utils.fromJson(responseBody,SearchAPIResponse.class);
			} else {
				SearchAPIError searchAPIError = (SearchAPIError) Utils
						.fromJson(responseBody, SearchAPIError.class);
				list.add(searchAPIError);
				//throw searchAPIError;
				return list;
			}
		} catch (JsonSyntaxException e) {
			//throw new SearchAPIError(responseBody, httpResponse.getStatusLine().getStatusCode());
			list.add(new SearchAPIError(responseBody, httpResponse.getStatusLine().getStatusCode()));
			return list;
		} finally {
